...
void main()
{
    ...
    // Usa a normal pra decidir qual partes das coordenadas usar pra mapear
    // a textura
    vec2 texcoord = vec2(0.0, 0.0);

    if (abs(dot(n.xyz, vec3(1.0, 0.0, 0.0))) > 0.9) {
        // X é o maior
        texcoord.x = position_modelspace_.z;
        texcoord.y = position_modelspace_.y;
    } else if (abs(dot(n.xyz, vec3(0.0, 1.0, 0.0))) > 0.9) {
        // Y é o maior
        texcoord.x = position_modelspace_.x;
        texcoord.y = position_modelspace_.z;
    } else {
        // Z é o maior
        texcoord.x = position_modelspace_.x;
        texcoord.y = position_modelspace_.y;
    ...
    ? pow(texture(baseTexture, texcoord).rgb, vec3(2.2))
    : material.Kd;
    vec3 ambientColor = bool(material.use_texture & UseBaseTexture)
    ? texture(baseTexture, texcoord).rgb
    : material.Ka;
    float q = bool(material.use_texture & UseShininessTexture)
    ? pow(2, texture(specularHighlightTexture, texcoord).r * 10.0)
    : material.q;
