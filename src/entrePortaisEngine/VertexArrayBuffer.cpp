...
namespace entre_portais
{
  ...
  void VertexArrayBuffer::Commit()
  {
    ...
    if (buffersToCreate != 0)
    {
      ...
      {
        auto vboBuffer = std::shared_ptr<VBOBuffer>(new VBOBuffer(buffersToBuild_[i], ids[i], weak_from_this()));
  ...
  VBOBuffer::VBOBuffer(BufferBuilder builder, unsigned int id, std::weak_ptr<VertexArrayBuffer> vao) : id_(id)
  {
    ...
    for (auto &attribute : builder.attributes_)
    {
      glVertexAttribPointer(
          attribute.index, attribute.size, attribute.type, attribute.normalized, attribute.stride, attribute.offset);
