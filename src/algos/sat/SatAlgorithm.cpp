...
bool SATAlgorithm::testCollisionSphereVsConvexPolyhedron(NarrowPhaseInfoBatch& narrowPhaseInfoBatch, uint32 batchStartIndex, uint32 batchNbItems) const {
    ...
    const glm::vec3 sphereCenter = sphereToPolyhedronSpaceTransform.getPosition();

    // Minimum penetration depth
    float minPenetrationDepth = DECIMAL_LARGEST;
    ...
    if (narrowPhaseInfoBatch.narrowPhaseInfos[batchIndex].reportContacts) {
        ...
        glm::vec3 minFaceNormalWorld = polyhedronToWorldTransform.getOrientation() * minFaceNormal;
        glm::vec3 contactPointSphereLocal = sphereToWorldTransform.getInverse().getOrientation() * (-minFaceNormalWorld * sphere->getRadius());
        glm::vec3 contactPointPolyhedronLocal = sphereCenter + minFaceNormal * (minPenetrationDepth - sphere->getRadius());
...
float SATAlgorithm::computePolyhedronFaceVsSpherePenetrationDepth(uint32 faceIndex, const ConvexPolyhedronShape* polyhedron,
    ...
    float penetrationDepth = sphereCenterToFacePoint.dot(faceNormal) + sphere->getRadius();
...
bool SATAlgorithm::testCollisionCapsuleVsConvexPolyhedron(NarrowPhaseInfoBatch& narrowPhaseInfoBatch, uint32 batchIndex) const {
    ...
    for (uint32 e = 0; e < polyhedron->getNbHalfEdges(); e += 2) {
        ...
        const glm::vec3 edgeVertex2 = polyhedron->getVertexPosition(polyhedron->getHalfEdge(edge.nextEdgeIndex).vertexIndex);
        const glm::vec3 edgeDirectionCapsuleSpace = polyhedronToCapsuleTransform.getOrientation() * (edgeVertex2 - edgeVertex1);

        const HalfEdgeStructure::Edge& twinEdge = polyhedron->getHalfEdge(edge.twinEdgeIndex);
        const glm::vec3 adjacentFace1Normal = polyhedronToCapsuleTransform.getOrientation() * polyhedron->getFaceNormal(edge.faceIndex);
        const glm::vec3 adjacentFace2Normal = polyhedronToCapsuleTransform.getOrientation() * polyhedron->getFaceNormal(twinEdge.faceIndex);
    ...
    const glm::vec3 capsuleSegAPolyhedronSpace = capsuleToPolyhedronTransform * capsuleSegA;
    const glm::vec3 capsuleSegBPolyhedronSpace = capsuleToPolyhedronTransform * capsuleSegB;

    glm::vec3 normalWorld = capsuleToWorld.getOrientation() * separatingAxisCapsuleSpace;
    if (isCapsuleShape1) {
    ...
    if (isMinPenetrationFaceNormal) {
        ...
        if (narrowPhaseInfoBatch.narrowPhaseInfos[batchIndex].reportContacts) {
            ...
            glm::vec3 contactPointCapsule = (polyhedronToCapsuleTransform * closestPointCapsuleInnerSegment) - separatingAxisCapsuleSpace * capsuleRadius;
...
float SATAlgorithm::computeEdgeVsCapsuleInnerSegmentPenetrationDepth(const ConvexPolyhedronShape* polyhedron, const CapsuleShape* capsule,
    ...
    float penetrationDepth = DECIMAL_LARGEST;
    ...
    if (outAxis.lengthSquare() >= float(0.00001)) {

        const glm::vec3 polyhedronCentroid = polyhedronToCapsuleTransform * polyhedron->getCentroid();
        const glm::vec3 pointOnPolyhedronEdge = polyhedronToCapsuleTransform * edgeVertex1;
...
float SATAlgorithm::computePolyhedronFaceVsCapsulePenetrationDepth(uint32 polyhedronFaceIndex, const ConvexPolyhedronShape* polyhedron,
    ...
    const float penetrationDepth = capsuleSupportPointToFacePoint.dot(outFaceNormalCapsuleSpace);
...
bool SATAlgorithm::computeCapsulePolyhedronFaceContactPoints(uint32 referenceFaceIndex, float capsuleRadius, const ConvexPolyhedronShape* polyhedron,
    ...
    Array<glm::vec3> planesPoints(mMemoryAllocator, 2);
    Array<glm::vec3> planesNormals(mMemoryAllocator, 2);
    ...
    do {
        ...
        glm::vec3 clipPlaneNormal = faceNormal.cross(edgeDirection);
    ...
    Array<glm::vec3> clipSegment = clipSegmentWithPlanes(capsuleSegAPolyhedronSpace, capsuleSegBPolyhedronSpace, planesPoints, planesNormals, mMemoryAllocator);
    ...
    for (uint32 i = 0; i < nbClipSegments; i++) {
        ...
        const float clipPointPenDepth = (planesPoints[0] - clipSegment[i]).dot(faceNormal);
...
bool SATAlgorithm::isMinkowskiFaceCapsuleVsEdge(const glm::vec3& capsuleSegment, const glm::vec3& edgeAdjacentFace1Normal,
    ...
    return capsuleSegment.dot(edgeAdjacentFace1Normal) * capsuleSegment.dot(edgeAdjacentFace2Normal) < float(0.0);
}

// Test collision between two convex polyhedrons
bool
SATAlgorithm::testCollisionConvexPolyhedronVsConvexPolyhedron(algo::shapes::Shape &shapeA, const glm::mat4 &transA, algo::shapes::Shape &shapeB,
    ...
    bool isCollisionFound = false;
        ...
        uint32 faceIndex1;
        float penetrationDepth1 = testFacesDirectionPolyhedronVsPolyhedron(polyhedron1, polyhedron2, polyhedron1ToPolyhedron2, faceIndex1);
...
//            lastFrameCollisionInfo->satMinAxisFaceIndex = faceIndex1;
        ...
        uint32 faceIndex2;
        float penetrationDepth2 = testFacesDirectionPolyhedronVsPolyhedron(polyhedron2, polyhedron1, polyhedron2ToPolyhedron1, faceIndex2);
...
//            lastFrameCollisionInfo->satMinAxisFaceIndex = faceIndex2;
        ...
        for (uint32 i=0; i < polyhedron1->getNbHalfEdges(); i += 2) {
            ...
            const glm::vec3 edge1A = polyhedron1ToPolyhedron2 * polyhedron1->getVertexPosition(edge1.vertexIndex);
            const glm::vec3 edge1B = polyhedron1ToPolyhedron2 * polyhedron1->getVertexPosition(polyhedron1->getHalfEdge(edge1.nextEdgeIndex).vertexIndex);
            ...
            for (uint32 j=0; j < polyhedron2->getNbHalfEdges(); j += 2) {
                ...
                const glm::vec3 edge2B = polyhedron2->getVertexPosition(polyhedron2->getHalfEdge(edge2.nextEdgeIndex).vertexIndex);
                ...
                if (testEdgesBuildMinkowskiFace(polyhedron1, edge1, polyhedron2, edge2, polyhedron1ToPolyhedron2)) {
                    ...
                    // Compute the penetration depth
                    const glm::vec3 polyhedron1Centroid = polyhedron1ToPolyhedron2 * polyhedron1->getCentroid();
                    float penetrationDepth = computeDistanceBetweenEdges(edge1A, edge2A, polyhedron1Centroid, polyhedron2->getCentroid(),
...
//            lastFrameCollisionInfo->satMinAxisFaceIndex = minFaceIndex;
            ...
            if (narrowPhaseInfoBatch.narrowPhaseInfos[batchIndex].reportContacts) {
                ...
                glm::vec3 closestPointPolyhedron1EdgeLocalSpace = polyhedron2ToPolyhedron1 * closestPointPolyhedron1Edge;

                // Compute the world normal
                glm::vec3 normalWorld = narrowPhaseInfoBatch.narrowPhaseInfos[batchIndex].shape2ToWorldTransform.getOrientation() * minEdgeVsEdgeSeparatingAxisPolyhedron2Space;
...
bool SATAlgorithm::computePolyhedronVsPolyhedronFaceContactPoints(bool isMinPenetrationFaceNormalPolyhedron1,
    ...
    const glm::vec3 axisIncidentSpace = referenceToIncidentTransform.getOrientation() * axisReferenceSpace;

    // Compute the world normal
    const glm::vec3 normalWorld = isMinPenetrationFaceNormalPolyhedron1 ? narrowPhaseInfoBatch.narrowPhaseInfos[batchIndex].shape1ToWorldTransform.getOrientation() * axisReferenceSpace :
    ...
    Array<glm::vec3> verticesTemp1(mMemoryAllocator, nbMaxElements);
    Array<glm::vec3> verticesTemp2(mMemoryAllocator, nbMaxElements);
    ...
    do {
        ...
        // Compute the normal of the clipping plane for this edge
        // The clipping plane is perpendicular to the edge direction and the reference face normal
        const glm::vec3 planeNormal = axisReferenceSpace.cross(edgeDirection);
    ...
    Array<glm::vec3>& clippedPolygonVertices = areVertices1Input ? verticesTemp2 : verticesTemp1;

    // We only keep the clipped points that are below the reference face
    const glm::vec3 referenceFaceVertex = referencePolyhedron->getVertexPosition(referencePolyhedron->getHalfEdge(firstEdgeIndex).vertexIndex);
    ...
    for (uint32 i=0; i < nbClipPolygonVertices; i++) {
        ...
        float penetrationDepth = (referenceFaceVertex - clippedPolygonVertices[i]).dot(axisReferenceSpace);
...
float SATAlgorithm::computeDistanceBetweenEdges(const glm::vec3& edge1A, const glm::vec3& edge2A,
                                                  ...
                                                  bool isShape1Triangle, glm::vec3& outSeparatingAxisPolyhedron2Space) const {

    RP3D_PROFILE("SATAlgorithm::computeDistanceBetweenEdges", mProfiler);

    // If the two edges are parallel
    if (areParallelglm::vecs(edge1Direction, edge2Direction)) {
    ...
    glm::vec3 axis = edge1Direction.cross(edge2Direction).getUnit();
...
float SATAlgorithm::testSingleFaceDirectionPolyhedronVsPolyhedron(const ConvexPolyhedronShape* polyhedron1,
    ...
    const glm::vec3 faceNormalPolyhedron2Space = polyhedron1ToPolyhedron2.getOrientation() * faceNormal;
    ...
    // Compute the penetration depth
    const glm::vec3 faceVertex = polyhedron1ToPolyhedron2 * polyhedron1->getVertexPosition(face.faceVertices[0]);
    float penetrationDepth = (faceVertex - supportPoint).dot(faceNormalPolyhedron2Space);

    return penetrationDepth;
}

// Test all the normals of a polyhedron for separating axis in the polyhedron vs polyhedron case
float SATAlgorithm::testFacesDirectionPolyhedronVsPolyhedron(const ConvexPolyhedronShape* polyhedron1,
                                                               const ConvexPolyhedronShape* polyhedron2,
                                                               const Transform& polyhedron1ToPolyhedron2,
...
bool SATAlgorithm::testEdgesBuildMinkowskiFace(const ConvexPolyhedronShape* polyhedron1, const HalfEdgeStructure::Edge& edge1,
    ...
    const glm::vec3 a = polyhedron1ToPolyhedron2.getOrientation() * polyhedron1->getFaceNormal(edge1.faceIndex);
    const glm::vec3 b = polyhedron1ToPolyhedron2.getOrientation() * polyhedron1->getFaceNormal(polyhedron1->getHalfEdge(edge1.twinEdgeIndex).faceIndex);
    ...
    const glm::vec3 edge1Vertex2 = polyhedron1->getVertexPosition(polyhedron1->getHalfEdge(edge1.twinEdgeIndex).vertexIndex);
    const glm::vec3 bCrossA = polyhedron1ToPolyhedron2.getOrientation() * (edge1Vertex1 - edge1Vertex2);

    // Compute d.cross(c) using the edge direction
    const glm::vec3 edge2Vertex1 = polyhedron2->getVertexPosition(edge2.vertexIndex);
    const glm::vec3 edge2Vertex2 = polyhedron2->getVertexPosition(polyhedron2->getHalfEdge(edge2.twinEdgeIndex).vertexIndex);
...
bool SATAlgorithm::testGaussMapArcsIntersect(const glm::vec3& a, const glm::vec3& b, const glm::vec3& c, const glm::vec3& d,
                                             const glm::vec3& bCrossA, const glm::vec3& dCrossC) const {

    RP3D_PROFILE("SATAlgorithm::testGaussMapArcsIntersect", mProfiler);

    const float cba = c.dot(bCrossA);
    const float dba = d.dot(bCrossA);
    const float adc = a.dot(dCrossC);
    const float bdc = b.dot(dCrossC);
